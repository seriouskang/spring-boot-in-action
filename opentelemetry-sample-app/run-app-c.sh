...
found_cont=$(podman ps --noheading --filter name=app-c --format "{{.Names}}")
if [ "${found_cont}" ]
then
...
podman run --rm -d --name app-c \
           -p 28080:8080 \
           -e TARGET_ONE_HOST=192.168.64.129:8080 \
           -e TARGET_TWO_HOST=192.168.64.129:18080 \
           ...
           -e OTEL_RESOURCE_ATTRIBUTES=service_name=app-c \
